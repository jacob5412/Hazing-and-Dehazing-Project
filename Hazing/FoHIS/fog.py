...
elevation, distance, angle = tk.elevation_and_distance_estimation(
    img, depth, const.CAMERA_VERTICAL_FOV, const.HORIZONTAL_ANGLE, const.CAMERA_ALTITUDE
)
...
if const.FT == 0:  # only haze: const.FT should be set to 0
    ...
    if const.CAMERA_ALTITUDE <= const.FT:
        ...
        distance_through_haze[idx2] = (
            (const.HT - const.FT) * distance / (const.CAMERA_ALTITUDE - elevation[idx2])
        )
        distance_through_haze_free[idx2] = (
            distance[idx2] - distance_through_haze[idx2] - distance_through_fog[idx2]
        )
